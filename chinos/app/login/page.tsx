    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        <div className="flex justify-end">
          <Link href="#" className="font-[var(--font-montserrat)] text-[0.75rem] text-[var(--color-red)] opacity-70 hover:opacity-100 transition-opacity">
            Forgot password?
          </Link>
        </div>
...
function RegisterForm() {
  ...
  async function onSubmit(values: RegisterValues) {
    try {
      ...
      toast.success("Account created. Welcome to Chinoss!");
      router.push("/");
    } catch (error) {
    ...
    <main className="min-h-screen bg-[var(--color-cream)] flex items-center justify-center px-4 py-24">
      <div className="w-full max-w-md flex flex-col gap-8">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          />
          <Link href="/" className="font-[var(--font-montserrat)] font-bold text-[1rem] tracking-[0.25em] uppercase text-[var(--color-red-deep)] no-underline">
            CHINOSS
          </Link>
        </div>
        ...
        <p className="text-center font-[var(--font-montserrat)] text-[0.75rem] text-[var(--color-red-deep)] opacity-50">
          {tab === "login" ? (
            <>
              ...
              <button onClick={() => setTab("register")} className="text-[var(--color-red)] opacity-100 underline cursor-pointer bg-transparent border-none font-[var(--font-montserrat)] text-[0.75rem]">
                Register here
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={() => setTab("login")} className="text-[var(--color-red)] opacity-100 underline cursor-pointer bg-transparent border-none font-[var(--font-montserrat)] text-[0.75rem]">
                Sign in here
              </button>
            </>
          )}
